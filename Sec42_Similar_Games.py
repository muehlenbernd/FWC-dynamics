...
# choose experiment ID 1 for Schmidt at al. 2001 (PD games)
# choose experiment ID 2 for Rankin at al. 2000 (SH games)
experiment_ID = 1
...
for trial in range(num_trials):
    ...
    for current_round in range(len(game_sequence)):
        ...
        for pair in agent_pairs:
            ...
            # always update agents, except for experiments 5-8 (Ahn et al. 2001), final 8 rounds
            if experiment_ID < 5 or current_round < 8:
...
# print round-by-round results for the experiments by Schmidt et al. (2001) and Ahn et al. (2001)
if experiment_ID ==1:
    ...
    for rate in sim_coop_rates:
        if experiment_ID < 5 or sim_coop_rates.index(rate) > 7:
            print round(rate,3), "\t",
    print
